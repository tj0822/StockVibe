...
class NewsAnalysis:
    ...
    def batch_summarize_texts(self, texts, max_length=200, num_beams=4, early_stopping=True):
        ...
        self.tokenizer_model_name = "C:\workspace\HfModels\kobart-summarization"
        ...
        with torch.no_grad():
            ...
            for i, pred in enumerate(predictions):
                ...
                if positive_prob >= 0.6:
                    # sentiment_results.append(f"긍정 😊 ({positive_prob:.2f})")
                    sentiment_results.append(f"😊")
                elif negative_prob >= 0.6:
                    # sentiment_results.append(f"부정 😡 ({negative_prob:.2f})")
                    sentiment_results.append(f"😡")
                else:
                    # sentiment_results.append(f"중립 🔘 ({positive_prob:.2f}, {negative_prob:.2f})")  # 애매하면 중립
    ...
    def batch_sentiment_analysis(self, news_df, batch_size=16):        
        ...
        self.sentiment_model_name = "C:\workspace\HfModels\korean_sentiment_analysis_kcelectra"  # 모델명을 올바르게 변경
