...
- **🏠 메인 대시보드** - 시그널/시뮬레이션/최적화/데이터 탭 통합 분석
- **📈 종목분석** - 개별 종목 차트/신호/뉴스 기반 상세 분석
- **💼 포트폴리오 관리** - 보유 종목, 거래 분석, 거래 이력 입력
- **⚙️ 개인화 설정** - 테마/기본값 등 사용자 환경 설정
...
2. 실시간 수익률 확인
3. 거래 분석 탭에서 승률/실현손익 확인
4. 거래 이력 입력 탭으로 내역 반영
```

### 시나리오 3: 전략 검증
```
1. 메인 대시보드 > 🎯 시뮬레이션 탭 선택
2. 관심 종목 + 전략 선택
3. 과거 데이터로 검증
4. 수익률/승률 확인
5. 최적 전략 선택
...
### 시나리오 4: 최적화 기반 파라미터 적용
```
1. 메인 대시보드 > ⚙️ 최적화 탭 실행
2. Top N 후보 비교
3. 후보 파라미터를 시뮬레이션 탭으로 적용
4. 시뮬레이션 탭에서 재검증
```
